...
public class ChoiceHandlerSuperadmin implements ChoiceHandler {
    ...
    public void handleChoice(Server server, String choice) throws MsgServerException {
        ...
        } else if (choice.toLowerCase().contains("make admin")) {
            String[] fields = choice.split(" ");
            if (fields.length < 3) throw new MsgServerException("User name is missing.");

            if (server.lookupMember(fields[2]) != null)
                server.updateMember(server.lookupMember(fields[2]),
                        UserFactory.getUser(UserRole.ADMIN.getDescriptor(), fields[2], false));
